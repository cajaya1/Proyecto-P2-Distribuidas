...
// ==================== API SERVICES ====================
// Detectar si estamos en desarrollo o producción (Docker)
const isDev = import.meta.env.DEV;
const API_BASE = isDev ? 'http://localhost:8082' : '';
const AUTH_API = isDev ? 'http://localhost:8081' : '';

const api = {
  login: async (cedula: string, password: string): Promise<string> => {
    const url = isDev ? `${AUTH_API}/auth/login` : '/api/auth/login';
  ...
  actualizarPedido: async (token: string, id: number, updates: Partial<Pedido>): Promise<Pedido> => {
    const url = isDev ? `${API_BASE}/api/pedidos/${id}` : `/api/pedidos/${id}`;
    const res = await fetch(url, {
      method: 'PATCH',
      headers: { 
        'Content-Type': 'application/json',
...
function App() {
  ...
  const handleLogin = async (cedula: string, password: string, rol: Usuario['rol']) => {
    ...
    try {
      ...
      // Para demostración, permitir login simulado
      setToken('demo-token');
      setUser({
        id: 1,
        cedula,
    ...
    <div style={{ ...styles.container, ...styles.loginBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ ...styles.card, width: '420px' }}>
        ...
        <div style={{ marginTop: '24px', padding: '16px', backgroundColor: '#f3f4f6', borderRadius: '8px', fontSize: '13px' }}>
          ...
          Cédula: 1724562440 | Contraseña: password123
        </div>
      </div>
    </div>
  );
      ...
      {selectedPedido && (
        <div style={{ ...styles.card, marginBottom: '24px' }}>
          ...
          <div style={{ marginTop: '16px', height: '350px', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            >
              ...
              <Marker position={[-0.2295, -78.5243]} icon={destinoIcon}>
                <Popup>
                  <strong>📍 Destino de Entrega</strong><br/>
                  {selectedPedido.direccionEntrega}
                </Popup>
              </Marker>
              {/* Marcador del repartidor (simulado moviéndose) */}
          ...
          <div style={{ marginTop: '16px', padding: '16px', backgroundColor: '#dbeafe', borderRadius: '8px' }}>
            <strong>Estado:</strong> {selectedPedido.estado} | <strong>Dirección:</strong> {selectedPedido.direccionEntrega}
          </div>
        </div>
      )}
...
function RepartidorDashboard({ token: _token, user: _user }: { token: string; user: Usuario }) {
  const [asignaciones, setAsignaciones] = useState<Pedido[]>([
    { id: 4, clienteId: 1750123456, direccionEntrega: 'Av. República E7-123, Quito', estado: 'ASIGNADO', tarifa: 18.00, repartidorId: 1 },
    { id: 5, clienteId: 1720987654, direccionEntrega: 'Calle Colón y 6 de Diciembre', estado: 'EN_CAMINO', tarifa: 22.50, repartidorId: 1 }
  ]);
        ...
        {asignaciones.map((pedido) => (
          <div key={pedido.id} style={{ ...styles.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ margin: '0 0 4px 0', color: '#6b7280' }}>📍 {pedido.direccionEntrega}</p>
              <p style={{ margin: 0, color: '#10b981', fontWeight: 'bold' }}>💰 ${pedido.tarifa.toFixed(2)}</p>
            </div>
...
function SupervisorDashboard({ token: _token }: { token: string }) {
  const [pedidos, setPedidos] = useState<Pedido[]>([
    { id: 1, clienteId: 1724562440, direccionEntrega: 'Av. Amazonas N36-152', estado: 'EN_CAMINO', tarifa: 15.50, repartidorId: 1 },
    { id: 2, clienteId: 1750123456, direccionEntrega: 'Calle García Moreno 456', estado: 'PENDIENTE', tarifa: 12.00 },
    { id: 3, clienteId: 1720987654, direccionEntrega: 'Mall El Jardín', estado: 'ENTREGADO', tarifa: 8.50, repartidorId: 2 },
  ...
  const [repartidores] = useState<Repartidor[]>([
    ...
    { id: 2, nombre: 'María García', cedula: '1723456789', estado: 'DISPONIBLE', vehiculoTipo: 'VEHICULO_LIVIANO', ubicacion: { lat: -0.19, lng: -78.48 } },
    { id: 3, nombre: 'Juan Pérez', cedula: '1734567890', estado: 'EN_RUTA', vehiculoTipo: 'CAMION', ubicacion: { lat: -0.17, lng: -78.46 } }
  ]);
  ...
  const exportarCSV = () => {
    const headers = ['ID', 'Cliente', 'Dirección', 'Estado', 'Tarifa', 'Repartidor'];
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2 style={{ margin: 0, fontSize: '28px', color: '#1f2937' }}>👨‍💼 Panel de Supervisor</h2>
      ...
      {/* Mapa de Flota */}
      <div style={{ ...styles.card, marginBottom: '24px' }}>
        <h3 style={{ margin: '0 0 16px 0' }}>🗺️ Mapa de Flota en Tiempo Real</h3>
        <div style={{ height: '300px', backgroundColor: '#e5e7eb', borderRadius: '8px', position: 'relative' }}>
          {repartidores.map((rep, idx) => (
            <div key={rep.id} style={{ 
              ...
              {rep.vehiculoTipo === 'MOTORIZADO' ? '🏍️' : rep.vehiculoTipo === 'VEHICULO_LIVIANO' ? '🚗' : '🚚'} {rep.nombre}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={styles.card}>
        <h3 style={{ margin: '0 0 16px 0' }}>📋 Lista de Pedidos</h3>
...
// ==================== GERENTE DASHBOARD ====================
function GerenteDashboard({ token: _token }: { token: string }) {
  ...
  const kpisPorZona = [
    ...
    { zona: 'Cumbayá', pedidos: 45, entregados: 39, ingresos: 759.00 }
  ];
          ...
          <div style={{ height: '200px', display: 'flex', alignItems: 'end', gap: '8px', padding: '16px' }}>
            {[65, 78, 82, 90, 85, 92, 88].map((valor, idx) => (
              <div key={idx} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Día {idx + 1}</span>
              </div>
            ))}
          </div>
        </div>
